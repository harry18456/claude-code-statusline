...
| 7d 節奏 | `▲7%` / `▼3%` / `≈` | 對 `seven_day` 配額，依**每日線性預期**使用量比對實際用量：`expected = ceil(elapsed / 1 天) × (100/7)`，亦即第 1 天預期 14.29%、第 2 天 28.57%、…、第 7 天 100%。跳階點對齊 `resets_at` 鐘點，非日曆午夜。偏離 > 5% 顯示紅色 `▲<N>%`（超支）或灰色 `▼<N>%`（落後）；偏離 ≤ 5% 顯示灰色 `≈`。僅在缺 `resets_at` 或視窗已過期時不顯示；`5h` 永不顯示此指示。ASCII 模式降級為 `^<N>%` / `v<N>%` / `~` |
