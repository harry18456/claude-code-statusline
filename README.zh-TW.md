...
| ⚠ 警告 | `⚠` | 上下文 ≥ 90% 時才出現 |
| 視窗大小 | `200k` / `1M` | 僅在模型名稱未包含此資訊時顯示。當 session 已跨過 200k token 貴價門檻（input 2×、output 1.5×）時 `1M` 會變**紅色** |
...
| 7d 節奏 | `▲7%` / `▼3%` / `≈` | 對 `seven_day` 配額，依線性預期使用量比對實際用量：偏離 > 5% 顯示紅色 `▲<N>%`（超支）或灰色 `▼<N>%`（落後）；偏離 ≤ 5% 顯示灰色 `≈`。剩餘時間 < 10% 視窗長度或缺 `resets_at` 時不顯示；`5h` 永不顯示此指示。ASCII 模式降級為 `^<N>%` / `v<N>%` / `~` |
